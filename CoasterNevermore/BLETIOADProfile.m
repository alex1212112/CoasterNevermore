...
-(IBAction)selectImagePressed:(id)sender {
    ...
    NSLog(@"state === %ld",(long)self.d.p.state);
    if (self.d.p.state != CBPeripheralStateConnected || !self.d.p) {
...
-(void) actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    NSInteger cancelIndex = actionSheet.numberOfButtons - 1;
...
//                    break;
        ...
        case 1: {
            switch (buttonIndex) {
                case 0: {
                    ...
                    [self uploadFileDidSelected:path];
                    break;
                }
                case 1: {
                    NSMutableString *path= [[NSMutableString  alloc] initWithString: [[NSBundle mainBundle] resourcePath]];
        ...
        case 2: {
            ...
            [self uploadFileDidSelected:fileName];
...
- (void)uploadFileDidSelected:(NSString *)filePath
{
    CSTUpdateFirmwareView *updateFirmwareView = (CSTUpdateFirmwareView *)self.hud.customView;
    updateFirmwareView.progressView.progress = 0.5f;
    [self runMethodAfterDelay:60.0f withMethod:^{
        [self validateImage:filePath];
    }];
}
...
-(void) programmingTimerTick:(NSTimer *)timer {
  ...
  //  self.progressDialog.label1.text = [NSString stringWithFormat:@"%0.1f%%",(float)((float)self.iBlocks / (float)self.nBlocks) * 100.0f];
   ...
   // float secondsLeft = (float)(self.nBlocks - self.iBlocks) * secondsPerBlock;
    ...
    {
          updateFirmwareView.progressView.progress = (float)((float)self.iBlocks / (float)self.nBlocks) / 2 + 0.5f;
    }
...
-(void)deviceDisconnected:(CBPeripheral *)peripheral {
    if ([peripheral isEqual:self.d.p] && self.inProgramming) {
       // [self.progressDialog dismissWithClickedButtonIndex:0 animated:YES];
        ...
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"固件升级失败" message:@"蓝牙断开连接" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
        alertView.tag = 0;
        [alertView show];
        self.inProgramming = NO;
        self.canceled = TRUE;
...
-(BOOL)validateImage:(NSString *)filename {
    ...
    NSLog(@"self === %x",self.imgVersion & 0x01);
    ...
    if ([self isCorrectImage]) [self uploadImage:filename];
       ...
       // UIAlertView *wrongImage = [[UIAlertView alloc]initWithTitle:@"Wrong image type !" message:[NSString stringWithFormat:@"Image that was selected was of type : %c, which is the same as on the peripheral, please select another image",(self.imgVersion & 0x01) ? 'B' : 'A'] delegate:nil cancelButtonTitle:@"Cancel" otherButtonTitles: nil];
        UIAlertView *wrongImage = [[UIAlertView alloc]initWithTitle:@"升级失败" message:[NSString stringWithFormat:@"升级文件错误:当前升级文件为%c,请使用另外一个版本",(self.imgVersion & 0x01) ? 'B' : 'A'] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [wrongImage show];
    }
    return NO;
}
...
-(BOOL) isCorrectImage {
    ...
    NSLog(@"file === %x",imgHeader.ver & 0x01);
...
//    [[GHCache shareCache] clearCacheWithFile:[self.filePath lastPathComponent]];
//    
...
//    
//    complete = [[UIAlertView alloc]initWithTitle:@"固件升级完成" message:@"固件升级成功,请稍候杯垫重启" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
//    [complete show];
}
